...
At the top of both `app.js` and `submit.js`:
```javascript
...
3. Set the start command: `uvicorn server:app --host 0.0.0.0 --port 10000`
4. Add environment variables: `PINECONE_API_KEY`, `GOOGLE_API_KEY`
5. Copy your Render URL into `app.js` and `submit.js` as `BACKEND_URL`

### Frontend
The frontend is plain HTML/CSS/JS — host it anywhere:
- GitHub Pages
- Netlify (drag and drop the folder)
- Vercel

### Note on SQLite in production
`feedback_blocks.db` is a local file. On Render's free tier, the filesystem resets on each deploy. For persistent feedback storage in production, consider migrating the block system to a hosted database (PostgreSQL, PlanetScale, etc.) or Pinecone itself.
...
In both `app.js` and `submit.js`, comment out the production URL and uncomment the local one:
...
Open `index.html` directly in your browser. No extra server needed for the frontend — just double-click the file or drag it into your browser.
...
By default it runs the test query at the bottom of `main.py`:
```python
...
| Problem | Fix |
|---------|-----|
| `PINECONE_API_KEY not found` | Make sure `.env` file exists and `python-dotenv` is installed |
